<script lang="ts">
  let { title = 'Nothing here yet', description = '', icon = 'empty' } = $props();

  const icons: Record<string, string> = {
    empty: 'M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0v5a2 2 0 01-2 2H6a2 2 0 01-2-2v-5m16 0h-2.586a1 1 0 00-.707.293l-2.414 2.414a1 1 0 01-.707.293h-3.172a1 1 0 01-.707-.293l-2.414-2.414A1 1 0 006.586 13H4',
    rooms: 'M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4',
    discussions: 'M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z',
    docs: 'M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z',
    teams: 'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z'
  };
</script>

<div class="flex flex-col items-center justify-center py-16 px-4 text-center animate-scale-in">
  <div class="w-16 h-16 rounded-2xl bg-[var(--dp-bg-surface)] flex items-center justify-center mb-4">
    <svg class="w-8 h-8 text-[var(--dp-text-muted)]" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
      <path stroke-linecap="round" stroke-linejoin="round" d={icons[icon] || icons.empty} />
    </svg>
  </div>
  <h3 class="text-lg font-semibold text-[var(--dp-text)] mb-1">{title}</h3>
  {#if description}
    <p class="text-sm text-[var(--dp-text-muted)] max-w-sm">{description}</p>
  {/if}
</div>
